...
controller.A.onEvent(ControllerButtonEvent.Pressed, function () {
    ...
    Missile.setPosition(Tank.x, Tank.y)
    Missile.setVelocity(0, -250)
})
sprites.onOverlap(SpriteKind.Projectile, SpriteKind.Bomb, function (sprite, otherSprite) {
    Missile.destroy(effects.spray, 0)
})
sprites.onDestroyed(SpriteKind.Enemy, function (sprite) {
    info.changeScoreBy(Math.abs(EnemyPlane.vx) * 0.1)
    music.baDing.play()
})
function spawnEnemy (mySprite: Sprite, num: number) {
    for (let index = 0; index < num; index++) {
        pause(randint(0, 0))
        randomiser = randint(0, 1)
        if (randomiser == 0) {
            randomiser = -1
            enemySpawnPosition = 160
        } else {
        ...
        EnemyPlane = sprites.create(img`
            ...
            . . . . . . . . . . . . . . . . 
            `, SpriteKind.Enemy)
        EnemyPlane.setVelocity(randint(40, 70) * randomiser, 0)
        EnemyPlane.setPosition(enemySpawnPosition, randint(0, 20))
    }
}
sprites.onOverlap(SpriteKind.Projectile, SpriteKind.Enemy, function (sprite, otherSprite) {
    EnemyPlane.destroy(effects.spray, 0)
})
...
Tank = sprites.create(img`
    ...
    `, SpriteKind.Player)
Tank.setPosition(80, 115)
controller.moveSprite(Tank, 200, 0)
for (let index = 0; index < 200; index++) {
    spawnEnemy(EnemyPlane, 2)
    pause(2000)
}
